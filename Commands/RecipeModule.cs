...
namespace ACNH.LookupBot {
  ...
  public sealed class RecipeModule : ModuleBase<SocketCommandContext> {
    [Command("recipeLang")]
    [Alias("rl")]
    [Summary("DIY recipe codes by item name using an NHSE language code (first arg), then name text.")]
    ...
    [Command("recipe")]
    [Alias("ri", "searchDIY")]
    [Summary("DIY recipe codes by item name using NHSE default language strings.")]
    public async Task SearchItemsAsync(
        [Summary("Item name / item substring")]
        [Remainder]
        string itemName) {
    ...
    private async Task PrintItemsAsync(string itemName, IReadOnlyList<ComboItem> strings) {
      ...
      foreach (var item in strings) {
        ...
        string msg = $"{item.Value:X4} {item.Text}: Recipe order code: {recipeID:X3}000016A2";
      ...
      foreach (var item in items) {
        ...
        string line = $"{item.Value:X4} {item.Text}: Recipe order code: {recipeID:X3}000016A2";
        matches.Add(line);
      }
